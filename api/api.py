...
for i in range (PRIMEIRO_POKEMON, ULTIMO_POKEMON + 1):
	JSONContent = requests.get(URL + str(i)).json()
	id = JSONContent['id']
...
dataset.columns = ['Id', 'Name', 'Type_Primary', 'Type_Secondary', 'Primary_Ability', 'Secondary_Ability', 'Hidden_Ability', 'HP', 'Attack', 'Defense', 'Special_Attack', 'Special_Defense', 'Speed', 'Total']
